...
public class GunScript : MonoBehaviour {
	...
	private PlayerMechanicsScript pMechS;
	[Range(0, 10)] public float shotCost;
	...
	void Awake(){
		...
		pMechS = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerMechanicsScript>();
	...
	private void ShootMethod(){
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 15){
			...
			if (pMechS.stressLevel < pMechS.maxStress) { 
				...
				waitTillNextFire = 1;
				pMechS.UpdateStressLevel(shotCost);
			}
